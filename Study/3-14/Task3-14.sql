...
SELECT goods_table.goods_code, goods_table.goods_name, goods_table.price
FROM goods_table
WHERE goods_table.price >= 2000;

-- 2. 店舗情報を店舗名のABC順に抽出して下さい。
SELECT store_table.store_name, store_table.store_nameAbc
FROM store_table
ORDER BY store_table.store_nameAbc;
...
ORDER BY store_table.store_code, goods_table.goods_code;
 

-- 4. 商品テーブルから全商品の価格の平均値を抽出して下さい。
SELECT
  goods_table.goods_code,
  goods_table.goods_name,
  ROUND(AVG(goods_table.price), 0) 
FROM goods_table
GROUP BY
  goods_table.goods_code,
  goods_table.goods_name
ORDER BY goods_table.goods_code;

-- 5. 店舗コード（'EA03'）の店舗に関連する在庫情報（商品コード、在庫数）を取得して下さい。
SELECT stock_table.goods_code, stock_table.quantity
FROM stock_table
WHERE stock_table.store_code = 'EA03'
ORDER BY stock_table.goods_code;

-- 6. 商品テーブルに「商品コード='M001'、商品名='マフラー'、価格=4500円、更新日付=本日日付」のデータを追加して下さい。。※実行後の「データ出力」画面も貼付すること。
INSERT INTO goods_table (goods_code, goods_name, price, update_date)
...
UPDATE stock_table
SET quantity = 50
WHERE stock_table.quantity >= 20;

-- 「データ出力」画面
SELECT *
FROM stock_table
WHERE stock_table.quantity = 50
ORDER BY stock_table.store_code, stock_table.goods_code;

-- 9. 在庫テーブルの商品（'S987'）かつ、店舗（'EA01'）の在庫数を10増やす更新を行って下さい。※実行後のSELECT結果も貼付すること。
UPDATE stock_table
SET quantity = stock_table.quantity + 10
WHERE stock_table.goods_code = 'S987'
  AND stock_table.store_code = 'EA01';
...
WHERE stock_table.goods_code = 'Z939'
  AND stock_table.store_code = 'EA04';
